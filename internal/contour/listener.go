		...
		if vh.TCPProxy == nil {
			...
			if vh.AuthorizationService != nil {
				authFilter = envoy.FilterExternalAuthz(
					envoy.Clustername(&vh.AuthorizationService.Cluster),
					vh.AuthorizationTimeout,
				)
			}
			...
			filters = envoy.Filters(
				envoy.HTTPConnectionManagerBuilder().
					...
					AddFilter(&http.HttpFilter{Name: wellknown.Gzip}).
					AddFilter(&http.HttpFilter{Name: wellknown.GRPCWeb}).
					AddFilter(authFilter).
					// XXX(jpeach) The HTTP Router filter has to come last.
					AddFilter(&http.HttpFilter{Name: wellknown.Router}).
